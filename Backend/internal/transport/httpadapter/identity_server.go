...
func NewIdentityHandler(service *identity.Service) http.Handler {
	...
	mux.HandleFunc("POST /internal/v1/identity/login", func(w http.ResponseWriter, r *http.Request) {
		...
		var err error
		if req.Provider=="password" {session,err=service.LoginPassword(r.Context(),req.GameID,req.AccountName,req.Credential,req.DeviceID)} else {session,err=service.LoginGuest(r.Context(),req.GameID,req.DeviceID)}
		if err != nil {
	...
	mux.HandleFunc("POST /internal/v1/identity/authenticate", func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			if status==401 {writeJSON(w,http.StatusOK,map[string]any{"valid":false,"errorCode":code})} else {writeError(w,status,code,nil)}
			return
		}
