...
func registerIdentityOnline(mux *http.ServeMux,service *identity.Service) {
 mux.HandleFunc("GET /internal/v1/identity/probe",func(w http.ResponseWriter,r *http.Request){if err:=service.Probe(r.Context());err!=nil {writeOnlineDomainError(w,err);return};writeJSON(w,200,map[string]bool{"ready":true})})
 mux.HandleFunc("POST /internal/v1/identity/refresh",func(w http.ResponseWriter,r *http.Request){
  token,ok:=internalRefreshToken(w,r);if !ok {return};session,err:=service.Refresh(r.Context(),token);if err!=nil {writeOnlineDomainError(w,err);return};writeIdentitySession(w,session)
 })
 mux.HandleFunc("POST /internal/v1/identity/logout",func(w http.ResponseWriter,r *http.Request){
  token,ok:=internalRefreshToken(w,r);if !ok {return};if err:=service.Logout(r.Context(),token);err!=nil {writeOnlineDomainError(w,err);return};w.WriteHeader(204)
 })
}
func internalRefreshToken(w http.ResponseWriter,r *http.Request)(string,bool) {
 ...
 err:=gateway.DecodeOnlineJSON(r,&req,[]string{"refreshToken"},[]string{"refreshToken"});if err!=nil || req.RefreshToken=="" {writeError(w,400,"INVALID_REQUEST",nil);return "",false};return req.RefreshToken,true
}
