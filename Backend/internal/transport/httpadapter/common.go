...
func onlineDomainError(err error) error {
	...
	if errors.As(err, &app) {
		return gateway.ServiceError(app.Code)
	}
	switch {
	case errors.Is(err, identity.ErrInvalidCredentials):
		return gateway.ServiceError("AUTH_INVALID_CREDENTIALS")
	case errors.Is(err, identity.ErrInvalidToken):
		return gateway.ServiceError("AUTH_SESSION_INVALID")
	case errors.Is(err, identity.ErrTokenExpired):
		return gateway.ServiceError("AUTH_TOKEN_EXPIRED")
	case errors.Is(err, identity.ErrInvalidInput):
		return gateway.ServiceError("INVALID_REQUEST")
	default:
		return gateway.ServiceError("SERVICE_UNAVAILABLE")
	}
}
func writeOnlineDomainError(w http.ResponseWriter, err error) {
