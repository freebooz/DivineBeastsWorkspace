...
type ProfileUpdater interface {
	UpdateDisplayNameIdempotent(context.Context, string, string, int64, string) (PlayerProfile, error)
}
...
func OnlineErrorStatus(err error) (int, string) {
	if errors.Is(err, ErrUnauthorized) {
		return 401, "AUTH_SESSION_INVALID"
	}
	...
	switch code {
	case "AUTH_SESSION_INVALID", "AUTH_INVALID_CREDENTIALS", "AUTH_TOKEN_EXPIRED", "AUTH_REFRESH_REPLAY":
		return 401, string(code)
	case "AUTH_FORBIDDEN", "AUTH_ACCOUNT_DISABLED":
		return 403, string(code)
	case "INVALID_REQUEST", "AUTH_PROVIDER_UNSUPPORTED":
		return 400, string(code)
	case "REQUEST_TOO_LARGE":
		return 413, string(code)
	case "RATE_LIMITED":
...
func writeOnlineError(w http.ResponseWriter, err error) {
	status, code := OnlineErrorStatus(err)
...
func DecodeOnlineJSON(r *http.Request, target any, allowed, required []string) error {
	...
	if len(data) > 65536 {
		return ServiceError("REQUEST_TOO_LARGE")
	}
	if !utf8.Valid(data) {
		return ServiceError("INVALID_REQUEST")
	}
...
func (a *api) probeOnline(w http.ResponseWriter, r *http.Request) {
	...
	defer cancel()
	ready := true
	for _, port := range []any{a.identity, a.playerData} {
		p, ok := port.(ProbePort)
		if !ok {
			ready = false
			continue
...
func (a *api) refreshOnline(w http.ResponseWriter, r *http.Request) {
	...
	port, ok := a.identity.(AuthenticationLifecycle)
	if !ok {
		writeOnlineError(w, ServiceError("SERVICE_UNAVAILABLE"))
		return
	}
	response, err := port.Refresh(r.Context(), token)
	if err != nil {
		writeOnlineError(w, err)
		return
	}
...
func (a *api) logoutOnline(w http.ResponseWriter, r *http.Request) {
	...
	if err := port.Logout(r.Context(), token); err != nil {
		writeOnlineError(w, err)
		return
	}
	w.Header().Set("Cache-Control", "no-store")
...
func readRefreshToken(r *http.Request) (string, error) {
	...
	if err := DecodeOnlineJSON(r, &req, []string{"refreshToken"}, []string{"refreshToken"}); err != nil {
		return "", err
	}
...
func (a *api) updateProfile(w http.ResponseWriter, r *http.Request, session AuthenticatedSession) {
	...
	if err := DecodeOnlineJSON(r, &req, []string{"displayName", "expectedRevision"}, []string{"displayName", "expectedRevision"}); err != nil {
		writeOnlineError(w, err)
		return
	}
	req.DisplayName = strings.TrimSpace(req.DisplayName)
	key := r.Header.Get("Idempotency-Key")
	if req.DisplayName == "" || utf8.RuneCountInString(req.DisplayName) > 24 || req.ExpectedRevision < 0 || strings.TrimSpace(key) == "" || utf8.RuneCountInString(key) > 128 {
		writeOnlineError(w, ServiceError("INVALID_REQUEST"))
		return
	}
	port, ok := a.playerData.(ProfileUpdater)
	if !ok {
		writeOnlineError(w, ServiceError("SERVICE_UNAVAILABLE"))
		return
	}
	profile, err := port.UpdateDisplayNameIdempotent(r.Context(), session.PlayerID, req.DisplayName, req.ExpectedRevision, key)
	if err != nil {
		writeOnlineError(w, err)
		return
	}
...
func writeProfile(w http.ResponseWriter, profile PlayerProfile) {
	if profile.OwnedCharacterIDs == nil {
		profile.OwnedCharacterIDs = []string{}
	}
	w.Header().Set("Cache-Control", "no-store")
...
func newAuthRateLimiter(limit int) *authRateLimiter {
	...
	return &authRateLimiter{limit: limit, windows: map[string]authWindow{}}
}
func (a *api) allowAuthentication(w http.ResponseWriter, r *http.Request) bool {
	...
	entry, exists := l.windows[host]
	if !exists && len(l.windows) >= 4096 {
		w.Header().Set("Retry-After", "60")
		writeOnlineError(w, ServiceError("RATE_LIMITED"))
		return false
	}
	if entry.start.IsZero() || now.Sub(entry.start) >= time.Minute {
		entry = authWindow{start: now}
	}
