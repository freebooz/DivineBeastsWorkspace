param(
    ...
    [string]$FlowTestDefinition = 'GamePlatformDefinition:foundation.flow@1'
)

# 对已经启用本插件的真实宿主执行 UE5.8 构建与 Automation；不新建或复制正式插件源码。
$ErrorActionPreference = 'Stop'
if ([string]::IsNullOrWhiteSpace($FlowTestDefinition)) { throw '必须提供真实流程测试主资产身份；预设名称不代表资产已经存在。' }
...
& $editorCommand $projectFile -unattended -nop4 -NullRHI -nosplash `
    ...
    '-TestExit=Automation Test Queue Empty' "-ReportExportPath=$reportRoot" "-GamePlatformFlowTestDefinition=$FlowTestDefinition" -stdout -FullStdOutLogOutput
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
...
if ($cases.Count -lt 42 -or @($cases | Where-Object { $_.state -ne 'Success' }).Count -ne 0) {
    throw "流程测试缺失或失败：实际发现 $($cases.Count) 项，要求至少 42 项全部成功。"
}
